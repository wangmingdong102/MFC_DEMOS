...
//		wcstombs(name, filename, filenameLength);
		size_t cSize;
		wcstombs_s(&cSize, name, (filenameLength + 1), filename, filenameLength);
		...
		// PROMISE: Return true if file is successfully opened, false if otherwise.
	{
		...
		size_t cSize;
		wcstombs_s(&cSize, &*(filename_.begin()), (filenameLength + 1), filename, filenameLength);
		...
		// PROMISE: Return true if file is successfully created and opened, false if otherwise.
	{
		...
		wcscpy_s(root->name_, _countof(root->name_), L"Root Entry");
	...
	int CompoundFile::DirectoryList(vector<vector<wchar_t> >& list, const wchar_t* path)
	{
		...
		for (size_t i = 0; i<maxChildren; ++i)
		{
			wcscpy_s(&*(name.begin()), _countof(name_2), currentDirectory_->children_[i]->self_->name_);
			list.push_back(name);
		}
		currentDirectory_ = previousDirectories_.back();
		previousDirectories_.pop_back();
	...
	bool CompoundFile::Create(const char* filename)
	{
		...
		mbstowcs_s(&cSize, wname, (filenameLength + 1), filename, filenameLength);
	...
	int CompoundFile::ChangeDirectory(const char* path)
	{
		...
		mbstowcs_s(&cSize, wpath, (pathLength + 1), path, pathLength);
		wpath[pathLength] = 0;
	...
	int CompoundFile::PresentWorkingDirectory(vector<char>& path)
	{
		...
		if (ret == SUCCESS)
		{
			...
			wcstombs_s(&cSize, &*(path.begin()), (pathLength), &*(wpath.begin()), pathLength);
			path[pathLength] = 0;
		}
		return ret;
	}
		...
		// EXPLAIN: property contains information on the type of property to be created.
	{
		...
		if (propertyname != 0)
		{
			...
			wchar_t name_2[32];
			wcscpy_s(property->name_, _countof(name_2),propertyname);
			delete[] propertyname;
...
namespace YExcel
{
	...
	const LargeString& LargeString::operator=(const char* str)
	{
		...
		size_t len = strlen(str);
		name_.resize(len + 1);
		strcpy_s(&*(name_.begin()), (len + 1), str);
		return *this;
	}
	const LargeString& LargeString::operator=(const wchar_t* str)
	{
		...
		size_t len = wcslen(str);
		wname_.resize(len + 1);
		wcscpy_s(&*(wname_.begin()), len+1, str);
		return *this;
	}
	...
	size_t LargeString::ContinueRead(const char* data, size_t size)
	{
		...
		if (unicode_ & 1)
		{
			...
			else
			{
				...
				mbstowcs_s(&cSize, &*(wname_.begin()) + strpos, size, &*(name.begin()), size);
				npos += size;
			}
		...
		else
		{
			...
			if (unicode & 1)
			{
				...
				size_t cSize;
				LittleEndian::ReadString(data, &*(name.begin()), npos, size);
				wcstombs_s(&cSize, &*(name_.begin()) + strpos, (strpos + size), &*(name.begin()), size);
				npos += size * SIZEOFWCHAR_T;
			}
			else
			{
	...
	BasicExcelWorksheet* BasicExcel::AddWorksheet(int sheetIndex)
	{
		...
		{
			char sname[50];
			sprintf_s(sname, sizeof(sname), "Sheet%d", sheetNo++);
	...
	bool BasicExcel::GetSheetName(size_t sheetIndex, char* name)
	{
		if (!(workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1))
		{
			strcpy_s(name, strlen(workbook_.boundSheets_[sheetIndex].name_.name_) + 1, workbook_.boundSheets_[sheetIndex].name_.name_);
			return true;
		}
		else return false;
	}
	...
	bool BasicExcel::GetSheetName(size_t sheetIndex, wchar_t* name)
	{
		if (workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1)
		{
			wcscpy_s(name, wcslen(workbook_.boundSheets_[sheetIndex].name_.wname_) + 1, workbook_.boundSheets_[sheetIndex].name_.wname_);
			return true;
		}
		else return false;
	}
	...
	bool BasicExcelWorksheet::GetSheetName(char* name)
	{
		if (!(excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1))
		{
			strcpy_s(name, strlen(excel_->workbook_.boundSheets_[sheetIndex_].name_.name_) +1, excel_->workbook_.boundSheets_[sheetIndex_].name_.name_);
			return true;
		}
		else return false;
	}
	...
	bool BasicExcelWorksheet::GetSheetName(wchar_t* name)
	{
		if (excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1)
		{
			wcscpy_s(name, wcslen(excel_->workbook_.boundSheets_[sheetIndex_].name_.wname_) + 1, excel_->workbook_.boundSheets_[sheetIndex_].name_.wname_);
			return true;
		}
		else return false;
	}
	...
	bool BasicExcelCell::Get(char* str) const
	{
		...
		{
			if (str_.empty()) *str = '\0';
			else strcpy_s(str, strlen(&*(str_.begin())) +1, &*(str_.begin()));
			return true;
		}
		else return false;
	}
	...
	bool BasicExcelCell::Get(wchar_t* str) const
	{
		if (type_ == WSTRING)
		{
			...
			else wcscpy_s(str, wcslen(&*(wstr_.begin())) + 1, &*(wstr_.begin()));
			return true;
		}
		else return false;
	}
	...
	void BasicExcelCell::SetWString(const wchar_t* str)
	{
		...
		if (length > 0)
		{
			...
			wcscpy_s(&*(wstr_.begin()), (length + 1), str);
			str_.clear();
		}
		else EraseContents();
	}
